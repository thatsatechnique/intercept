...
def start_rf_fax() -> Response:
    ...
    with app_module.rf_fax_lock:
        ...
        # Validate rtl_tcp params BEFORE branching on their values
        ...
        # OOK PWM timing parameters with bounds validation
        try:
            short_pulse = validate_positive_int(data.get('short_pulse', 300), 'short_pulse', max_val=10000)
            long_pulse = validate_positive_int(data.get('long_pulse', 600), 'long_pulse', max_val=20000)
            reset_limit = validate_positive_int(data.get('reset_limit', 8000), 'reset_limit', max_val=100000)
            gap_limit = validate_positive_int(data.get('gap_limit', 5000), 'gap_limit', max_val=100000)
            tolerance = validate_positive_int(data.get('tolerance', 150), 'tolerance', max_val=5000)
            min_bits = validate_positive_int(data.get('min_bits', 64), 'min_bits', max_val=10000)
            expected_lines = validate_positive_int(data.get('expected_lines', 11), 'expected_lines', max_val=1000)
        except ValueError as e:
            return jsonify({'status': 'error', 'message': str(e)}), 400
        ...
        try:
            ...
            app_module.rf_fax_process._stderr_thread = stderr_thread
...
def stop_rf_fax() -> Response:
    ...
    with app_module.rf_fax_lock:
        if app_module.rf_fax_process:
            ...
            # Save refs before clearing process
            parser_thread = getattr(app_module.rf_fax_process, '_parser_thread', None)
            stderr_thread = getattr(app_module.rf_fax_process, '_stderr_thread', None)

            # Close pipes to unblock readline-blocked threads
            _close_pipe(getattr(app_module.rf_fax_process, 'stdout', None))
            _close_pipe(getattr(app_module.rf_fax_process, 'stderr', None))
            ...
            _join_thread(parser_thread, timeout_s=2.0)
            _join_thread(stderr_thread, timeout_s=1.0)
